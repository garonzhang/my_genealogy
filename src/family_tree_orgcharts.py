...
def draw_node(member_dict, cur_node):
    ...
    for son_member_id in member_obj.sons_list[::-1]:
        ...
        child_node = MemberNode(son_member_id, son_member_obj.member_name, son_member_obj.sex,son_spouse_name,
                                son_member_obj.descent_no,className)
        cur_node.add_son(child_node)
        draw_node(member_dict, child_node)


def get_data_part(member_dict, first_member_id):
    first_member_obj = member_dict.get(first_member_id)

    first_node = MemberNode(first_member_id, first_member_obj.member_name, first_member_obj.sex, first_member_obj.spouse_name if first_member_obj.spouse_name is not None else "",
                            first_member_obj.descent_no,"middle-level")
